...
def parse_contents(contents, filename, date):
    ...
    try:
        if 'csv' in filename:
            ...
            print('Byebye')
            all_teams_df = create_table(io.StringIO(decoded.decode('utf-8')))
            print('Byebyehi')
            team_names = list(all_teams_df.columns.values)
            team_names.remove('Seconds')
        elif 'log' in filename:
            # Assume that the user uploaded a LOG file
            logdf= pd.read_csv(io.StringIO(decoded.decode('utf-8')), sep='^([^ ]+ +[^ ]+) +(.*)$', names=['useless','Time','Desc'], 
                index_col=False, engine='python')
            logdf.drop('useless' , axis = 1)
            logdf['Time'] =  pd.to_datetime(logdf['Time'], format='%Y/%m/%d %H:%M:%S,%f')
            start_time = logdf[logdf['Desc'].str.contains('Start Build Job', regex=False)]['Time'].values[0]
            end_time = logdf[logdf['Desc'].str.contains('build job finished', regex=False)]['Time'].values[0]
    ...
    return html.Div([
        ...
        dash_table.DataTable(
            data=all_teams_df[:100].to_dict('records'),
            columns=[{'name': i, 'id': i} for i in all_teams_df.columns]
        ),
...
def create_table(filename=None):
    ...
    global start_time,end_time
    var = ['Time','Pressure','Filter Status','Gas flow speed','Oxygen top','Oxygen 1','Oxygen 2',
    'Gas Temp','Platform','Build Chamber','Optical Bench','Collimator','Pump1',
    'Pump','Cabinet','Cabinet 2','Ambiance']
    var2 = ['Pressure','Filter Status','Gas flow speed','Oxygen top','Oxygen 1','Oxygen 2',
    'Gas Temp','Platform','Build Chamber','Optical Bench','Collimator','Pump1',
    'Pump','Cabinet','Cabinet 2','Ambiance']
    print("BYEHI")
    ...
    # dtype_dict = {c: np.float32 for c in var2}
    # dtype_dict['Time'] = 

    df = pd.read_csv((filename), sep = ";", usecols=lambda x: x in var, dtype={c: np.float32 for c in var2})
    print(df.head)
    print('hihihi2')
    print('hihihi')
    print('hihihi')
